...
import { getCreatePolicyRegistryIx, getPolicyRegistryPda } from "../policy";
...
import { AssetRegistry } from "../../programs/types";
import { BN, Idl, Program, Provider } from "@coral-xyz/anchor";
import { AssetRegistryIdl } from "../../programs/idls";
...
export const ASSET_REGISTRY_PROGRAM_ID = new PublicKey("DtrBDukceZpUnWmeNzqtoBQPdXW8p9xmWYG1z7qMt8qG");

export const getAssetRegistryProgram = (provider: Provider) => {
    return new Program(
        AssetRegistryIdl as Idl,
        ASSET_REGISTRY_PROGRAM_ID,
        provider
    ) as unknown as Program<AssetRegistry>;
}

export const getAssetRegistryPda = (assetMint: string) => {
...
export interface SetupAssetRegistryArgs {
    decimals: number;
    authority: PublicKey;
    delegate: PublicKey | null;
}

export async function getCreateAssetRegistryIx(
    payer: string,
    mint: string,
    createAssetRegistryArgs: SetupAssetRegistryArgs
): Promise<TransactionInstruction> {
    const provider = getProvider();
    const assetProgram = getAssetRegistryProgram(provider);
    const ix = await assetProgram.methods.createAssetRegistry(createAssetRegistryArgs)
        .accountsStrict({
            payer,
            assetMint: mint,
            assetRegistry: getAssetRegistryPda(mint),
...
export async function getTransferTokensIx(args: TransferTokensArgs): Promise<TransactionInstruction> {
    ...
    const ix = await assetProgram.methods.transferTokens(new BN(args.amount), new PublicKey(args.to))
        .accountsStrict({
            ...
            signer: getAssetRegistryPda(args.mint),
            from: args.from,
            ...
            policyRegistry: getPolicyRegistryPda(args.mint),
...
export async function getCreateTransactionApprovalIx(
    ...
    const ix = await assetProgram.methods.approveTransaction({
        ...
        .accountsStrict({
            ...
            transactionApprovalAccount: getTransactionApprovalAccountPda(args.mint),
            systemProgram: SystemProgram.programId,
            signer: args.delegate ? new PublicKey(args.delegate) : getAssetRegistryPda(args.mint),
        })
        .instruction();
    return ix;
}
...
export async function getSetupAssetRegistryIxs(
    payer: string,
    authority: string,
    delegate: string,
    setupAssetRegistryArgs: SetupAssetRegistryArgs
): Promise<ixReturn> {
    const mintKp = new Keypair();
    const mint = mintKp.publicKey;
    // get asset registry create ix
    const assetRegistryCreateIx = await getCreateAssetRegistryIx(
        payer,
        mint.toString(),
        setupAssetRegistryArgs,
    );
    // get policy registry create ix
    const policyRegistryCreateIx = await getCreatePolicyRegistryIx(
        payer,
        authority,
        delegate,
        mint.toString(),
    ...
    return {
        ixs: [
            assetRegistryCreateIx,
            policyRegistryCreateIx,

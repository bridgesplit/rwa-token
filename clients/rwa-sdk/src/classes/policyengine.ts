...
import {type AttachPolicyArgs, getAttachPolicyAccountIx} from '../policy_engine';
...
export class PolicyEngine {
	...
	async attachPolicy(policyArgs: AttachPolicyArgs): Promise<IxReturn> {
		const attachPolicyIx = await getAttachPolicyAccountIx(policyArgs, this.rwaClient.provider);

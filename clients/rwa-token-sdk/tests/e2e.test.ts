...
describe('e2e tests', () => {
	...
	test('attach transaction amount limit policy', async t => {
		...
		expect(txnId).toBeTruthy();
		console.log(
			'transaction amount limit policy: ',
			attachPolicy.signers[0].publicKey.toString(),
		);
	});
	...
	test('attach transaction amount velocity policy', async t => {
		...
		expect(txnId).toBeTruthy();
		console.log(
			'transaction amount velocity policy: ',
			attachPolicy.signers[0].publicKey.toString(),
		);
	});
	...
	test('attach transaction count velocity policy', async t => {
		...
		expect(txnId).toBeTruthy();
		console.log(
			'transaction count velocity policy: ',
			attachPolicy.signers[0].publicKey.toString(),
		);
	});
	...
	test('setup user', async t => {
		...
		const txnId = await sendAndConfirmTransaction(
			setup.provider.connection,
			new Transaction().add(...setupUser.ixs),
			[setup.payerKp, ...setupUser.signers],
		);
		expect(txnId).toBeTruthy();

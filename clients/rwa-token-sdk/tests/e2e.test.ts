...
describe("e2e tests", async () => {
	...
	test("add level to user account", async () => {
		const addLevelArgs: AddLevelToIdentityAccountArgs = {
			authority: setup.authority.toString(),
			payer: setup.payer.toString(),
			owner: setup.authority.toString(),
		...
		const issueIx =
      await rwaClient.identityRegistry.addIdentityLevelToUserAccount(addLevelArgs);
		const txnId = await sendAndConfirmTransaction(
			rwaClient.provider.connection,
			new Transaction().add(issueIx),
			[setup.payerKp, setup.authorityKp]
		...
		expect(txnId).toBeTruthy();
		console.log("Add level signature: ", txnId);
	});

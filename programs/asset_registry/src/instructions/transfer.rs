...
use policy_registry::{deserialize_and_enforce_policy, PolicyRegistry};
...
pub fn handler<'info>(
    ...
    if remaining_accounts.len() < policies.len() + TRANSFER_REMAINING_ACCOUNTS_LEN {
        return Err(AssetRegistryErrors::PolicyRegistrysMissing.into());
    }
    for (i, policy) in policies[..].iter().enumerate() {
        let tracker_account = &remaining_accounts[i + TRANSFER_REMAINING_ACCOUNTS_LEN];
        if tracker_account.key != *policy {
            return Err(AssetRegistryErrors::PolicyRegistryMismatch.into());
        }
        // evaluate policy
    ...
    let max_timeframe = ctx.accounts.policy_registry.max_timeframe;
    ctx.accounts
        .tracker_account

...
pub struct GenerateTransactionApproval<'info> {
    ...
    #[account(
        mut,
        constraint = authority.key() == asset_controller.transaction_approval_authority,
    )]
    pub authority: Signer<'info>,
    #[account()]
    pub owner: Signer<'info>,
    #[account(
        seeds = [asset_controller.asset_mint.key().as_ref(), ASSET_CONTROLLER_DATA_SEED.as_bytes()],
        bump,
    )]
    pub asset_controller: Box<Account<'info, AssetController>>,
    #[account(
        init,
...
pub fn handler(
    ...
    transaction_approval_account.asset_mint = ctx.accounts.asset_controller.asset_mint;

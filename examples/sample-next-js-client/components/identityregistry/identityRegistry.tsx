...
export const IdentityRegistry = () => {
  ...
  const [identityArgs, setIdentityArgs] = useState<IdentityRegistryArgs>(
    actions[0].args
  );
  const [selectedAction, setSelectedAction] = useState<
    Action<IdentityRegistryArgs>
  ...
  return (
    <div className="w-[700px]">
      ...
      <div className="py-4">
        <button
          type="submit"
          onClick={() => handleSubmit(identityArgs)}
          className="w-full cursor-pointer py-2 px-4 bg-[#ffc900] text-white font-semibold rounded-md hover:bg-[#c9a272] focus:outline-none border-4 border-black"
        >
          SUBMIT INSTRUCTIONS
        </button>
      </div>

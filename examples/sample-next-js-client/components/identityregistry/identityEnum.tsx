...
function DynamicComponent({ type, handleParentState }: { type: string, handleParentState: (key: string, value: string | number) => void }) {
    ...
    switch (type) {
        case ComponentTypes.SETUP_USER:
            componentToRender = <SetupUser message={ComponentTypes.SETUP_USER} args={IdentityTypes.SETUP_USER} onSubmit={handleSendingToParent} />;
            break;
        case ComponentTypes.ADD_IDENTITY_LEVEL_TO_USER:
            componentToRender = <LevelUser message={ComponentTypes.SETUP_USER} args={IdentityTypes.ADD_IDENTITY_LEVEL_TO_USER} onSubmit={handleSendingToParent} />;
            break;
        case ComponentTypes.REMOVE_LEVEL_FROM_USER:
            componentToRender = <LevelUser message={ComponentTypes.SETUP_USER} args={IdentityTypes.REMOVE_LEVEL_FROM_USER} onSubmit={handleSendingToParent} />;
            break;
        default:

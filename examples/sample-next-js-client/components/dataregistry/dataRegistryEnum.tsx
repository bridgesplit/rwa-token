import { DataAccountType } from "../../src";
...
function DynamicComponent({ type, handleParentState }: { type: string, handleParentState: (key: string, value: string | number | DataAccountType) => void }) {
    let componentToRender;

    const handleSendingToParent = (key: string, value: string | number | DataAccountType) => {
        handleParentState(key, value)
    }

    switch (type) {
        case ComponentTypes.SETUP_DATA_ACCOUNT:
            componentToRender = <CreateDataRegistry message={ComponentTypes.SETUP_DATA_ACCOUNT} args={IdentityTypes.SETUP_DATA_ACCOUNT} onSubmit={handleSendingToParent} />;
            break;
        case ComponentTypes.UPDATE_ASSETS_DATA_ACCOUNT:
            componentToRender = <UpdateDataRegistry message={ComponentTypes.UPDATE_ASSETS_DATA_ACCOUNT} args={IdentityTypes.UPDATE_ASSETS_DATA_ACCOUNT} onSubmit={handleSendingToParent} />;
            break;
        case ComponentTypes.DELEGATE_DATA_REGISTRY:
            componentToRender = <DelegateDataRegistryField message={ComponentTypes.DELEGATE_DATA_REGISTRY} args={IdentityTypes.DELEGATE_DATA_REGISTRY} onSubmit={handleSendingToParent} />;
            break;
        default:
